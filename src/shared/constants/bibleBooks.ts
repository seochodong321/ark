...
/** 성경 본문 문자열에서 책 이름을 추출한다 (예: "요 3:16" → "요한복음") */
export function extractBibleBook(scripture: string | null): string | null {
  if (!scripture) return null;
  const trimmed = scripture.trim();
  for (const book of BIBLE_BOOKS) {
    if (trimmed.includes(book)) return book;
  }
  // 축약 표기는 긴 것부터 검사 (삼상 vs 상, 고전 vs 전)
  const aliases = Object.keys(BIBLE_BOOK_ALIASES).sort(
    (a, b) => b.length - a.length,
  );
  for (const alias of aliases) {
    if (new RegExp(`(^|\\s)${alias}\\s?\\d`).test(trimmed)) {
      return BIBLE_BOOK_ALIASES[alias];
    }
  }
  return null;
}
