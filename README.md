...
### 자료실 (resources) — `/resources`
- 교회학교 기획안, 영상 템플릿, 디자인 소스 등 **소멸되기 쉬운 사역 자료를 나누고 보존**하는 게시판.
  자유게시판 대신 자료 나눔 전용으로 설계 — 보존 미션에 부합하고 모더레이션 부담이 적다.
- **회원 누구나 게시**(즉시 공개, draft 없음). 게시 시 씨앗 +3, 삭제 시 회수.
- 항목: 제목, 카테고리(기획·행사/디자인/영상·미디어/교육·교재/문서·양식/기타), 설명(Markdown), 태그, 첨부 파일.
- **파일 제한**: 개당 25MB·게시물당 5개, 허용 확장자만(zip·pdf·오피스·한글·이미지·미디어 등,
  실행 파일 차단). 크기 제한은 `storage.rules`에서도 강제. 상수: `shared/constants/uploads.ts`.
- 다운로드 수 집계(`downloadCount` — 규칙상 +1만 허용). 삭제 시 Storage 첨부 파일도 정리.
- 댓글·응원·북마크·신고 모두 지원 — `ContentType`에 `resource`를 추가해 기존 시스템에 통합.
...
### 회원 탈퇴 (settings)
- **인앱 자기주도 탈퇴** — 이메일 요청이 아니라 계정 설정(`/settings`)에서 직접.
- 흐름: 비밀번호 재인증(본인 확인) → 콘텐츠·프로필 데이터 삭제 → Auth 계정 삭제.
  Firestore 삭제를 먼저 끝내고 마지막에 Auth 계정을 지운다(계정이 사라지면 이후 쓰기가 거부되므로).
- 삭제 대상: 설교·간증·댓글·북마크·팔로우(본인) + 목회자/단체 인증 문서·연락처 + username 매핑 + users 문서.
  씨앗 거래 내역(불변 원장)과 타인이 남긴 카운터는 정리하지 않는다.
- 보안 규칙: users/usernames/pastors에 **본인 자가 삭제** 허용(기존 관리자 전용에서 확장).
- 확인 절차: 비밀번호 + "탈퇴합니다" 문구 입력 시에만 활성화.
...
| `resources` | auto | authorId, authorName·authorUsername·authorBadge(비정규화), title, description, category, tags[], files[](name·url·size·contentType·storagePath), **status**(published/hidden), viewCount, downloadCount, seedCount, commentCount |
...
- [x] 자료실 — 사역 자료 파일 나눔(25MB×5 제한), 카테고리, 다운로드 집계, 기존 참여 시스템 통합
